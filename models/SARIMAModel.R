...
SARIMAModel <- R6Class(
  ...
  private = list(
    ...
    .nsim = 10000,         ## models that are simulating forecasts need this
  ...
  public = list(
    ...
    fit = function(data) {
      ...
      for (row_idx in 1:private$.data$nrow) {
        ...
        private$.models[[row_idx]] <- auto.arima(y_ts,
                                                 seasonal = TRUE,
                                                 stationary = TRUE,
                                                 lambda = private$.lambda[[row_idx]],
                                                 trace=FALSE)
      }
    ...
    forecast = function(newdata = private$.data, steps) {
      ...
      for(sim in 1:private$.nsim) {
        for(model_idx in 1:length(private$.models)) {
          tmp_arima <- simulate(private$.models[[model_idx]],
                                nsim = steps,
                                lambda = private$.lambda[[model_idx]])
          # sim_forecasts$mutate(rows=model_idx, sims=sim, data=tmp_arima)
          sim_forecasts[model_idx, , sim] <- tmp_arima
        }
      }
